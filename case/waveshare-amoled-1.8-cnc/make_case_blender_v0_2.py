...
def rounded_box(name, width, depth, height, zbase, radius, segments=18):
    # CNC-friendly rounded-rectangle prism: rounded vertical corners, flat top/bottom.
    import math
    verts=[]
    centers=[(width/2-radius, depth/2-radius), (-width/2+radius, depth/2-radius), (-width/2+radius, -depth/2+radius), (width/2-radius, -depth/2+radius)]
    ...
    for p in outline:
        if not clean or abs(p[0]-clean[-1][0])>1e-6 or abs(p[1]-clean[-1][1])>1e-6:
            clean.append(p)
    n=len(clean)
    verts=[(x,y,zbase) for x,y in clean]+[(x,y,zbase+height) for x,y in clean]
    faces=[]
...
# ---------- bottom tray ----------
bottom = rounded_box('bottom_tray_cnc_aluminum', OUTER_W, OUTER_H, BOTTOM_Z, 0, CORNER_R, 24)
...
bevel_obj(bottom, 1.0, 8)

# ---------- top bezel ----------
top = rounded_box('top_bezel_cnc_aluminum', OUTER_W, OUTER_H, TOP_Z, BOTTOM_Z, CORNER_R, 24)
...
export_stl(bottom, 'waveshare_amoled_1p8_cnc_bottom_tray_v0_2.stl')
export_stl(top, 'waveshare_amoled_1p8_cnc_top_bezel_v0_2.stl')
# assembly preview
bpy.ops.object.select_all(action='DESELECT')
for obj in bpy.context.scene.objects:
    if not obj.hide_viewport and obj.type == 'MESH':
        obj.select_set(True)
bpy.ops.wm.stl_export(filepath=os.path.join(OUT_DIR, 'waveshare_amoled_1p8_cnc_assembly_preview_v0_2.stl'), export_selected_objects=True)

# save editable blend
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT_DIR, 'waveshare_amoled_1p8_cnc_case_v0_2.blend'))

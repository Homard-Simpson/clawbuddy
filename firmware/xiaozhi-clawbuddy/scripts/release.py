...
def _normalize_config_key(value: str) -> str:
    """Normalize strings so board/build names can be compared safely."""
    return "".join(ch.lower() for ch in value if ch.isalnum())



def _find_board_config(board_type: str, variant_name: Optional[str] = None) -> Optional[str]:
    """Find the corresponding CONFIG_BOARD_TYPE_xxx for the given board_type.

    Some boards expose multiple Kconfig symbols that map to the same BOARD_TYPE
    leaf (for example Waveshare ESP32-S3-ePaper-1.54 v1/v2), so when a variant
    name is available we use it to disambiguate.
    """
    ...
    for idx, line in enumerate(lines):
        ...
        for back_idx in range(idx - 1, -1, -1):
            ...
            if "if(CONFIG_BOARD_TYPE_" in back_line:
                matches.append(back_line.strip().split("if(")[1].split(")")[0])
                break

    if not matches:
        return None
    if len(matches) == 1 or not variant_name:
        return matches[0]

    normalized_variant = _normalize_config_key(variant_name)
    for match in matches:
        if normalized_variant in _normalize_config_key(match):
            return match
...
def release(board_type: str, config_filename: str = "config.json", *, filter_name: Optional[str] = None) -> None:
    ...
    for build in builds:
        ...
        board_type_config = _find_board_config(board_type, variant_name=name)

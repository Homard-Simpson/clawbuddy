...
void LcdDisplay::InitializeLcdThemes() {
    ...
    light_theme->set_chat_background_color(lv_color_hex(0xEEF3F8));
    light_theme->set_user_bubble_color(lv_color_hex(0x00C781));
    light_theme->set_assistant_bubble_color(lv_color_hex(0xFFFFFF));
    light_theme->set_system_bubble_color(lv_color_hex(0xE6FFF3));
    light_theme->set_system_text_color(lv_color_hex(0x0B1220));
    light_theme->set_border_color(lv_color_hex(0x00E08A));
    light_theme->set_low_battery_color(lv_color_hex(0xF04438));
    ...
    dark_theme->set_user_bubble_color(lv_color_hex(0x00D084));
    dark_theme->set_assistant_bubble_color(lv_color_hex(0x101827));
    dark_theme->set_system_bubble_color(lv_color_hex(0x08281A));
    dark_theme->set_system_text_color(lv_color_hex(0xD8FFF0));
    dark_theme->set_border_color(lv_color_hex(0x00F59B));
    dark_theme->set_low_battery_color(lv_color_hex(0xFF4D4D));
...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_style_border_color(top_bar_, lvgl_theme->system_bubble_color(), 0);
    lv_obj_set_flex_flow(top_bar_, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(top_bar_, LV_FLEX_ALIGN_SPACE_BETWEEN, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_set_scrollbar_mode(top_bar_, LV_SCROLLBAR_MODE_OFF);
    ...
    lv_obj_set_style_pad_top(content_, lvgl_theme->spacing(4), 0);
    lv_obj_set_style_pad_bottom(content_, lvgl_theme->spacing(8), 0);
    lv_obj_set_style_pad_left(content_, lvgl_theme->spacing(3), 0);
    lv_obj_set_style_pad_right(content_, lvgl_theme->spacing(3), 0);
    lv_obj_set_style_pad_row(content_, lvgl_theme->spacing(3), 0);
    lv_obj_set_flex_flow(content_, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_flex_align(content_, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START);
    lv_obj_set_scroll_dir(content_, LV_DIR_VER);
...
void LcdDisplay::SetPreviewImage(std::unique_ptr<LvglImage> image) {
    DisplayLockGuard lock(this);
    if (content_ == nullptr) {
        ESP_LOGE(TAG, "Preview image requested before chat content is initialized");
        return;
    }

    if (image == nullptr) {
        if (preview_image_ != nullptr && lv_obj_is_valid(preview_image_)) {
            lv_obj_t* bubble = lv_obj_get_parent(preview_image_);
            lv_obj_t* row = bubble != nullptr ? lv_obj_get_parent(bubble) : nullptr;
    ...
    if (preview_image_ == nullptr || !lv_obj_is_valid(preview_image_)) {
        lv_obj_t* row = lv_obj_create(content_);
        ...
        lv_obj_set_style_bg_color(bubble, lv_color_hex(0x111827), 0);
        lv_obj_set_style_bg_opa(bubble, LV_OPA_90, 0);
        lv_obj_set_style_border_width(bubble, 1, 0);
        lv_obj_set_style_border_color(bubble, lvgl_theme->border_color(), 0);
        lv_obj_set_style_pad_all(bubble, lvgl_theme->spacing(2), 0);
        lv_obj_set_scrollbar_mode(bubble, LV_SCROLLBAR_MODE_OFF);
        lv_obj_set_user_data(bubble, (void*)"image");

        preview_image_ = lv_image_create(bubble);
    ...
    lv_coord_t img_height = img_dsc->header.h;
    if (img_width <= 0 || img_height <= 0) {
        ESP_LOGW(TAG, "Invalid preview image dimensions: %ld x %ld", img_width, img_height);
        img_width = LV_HOR_RES;
        img_height = LV_VER_RES / 2;
    }

    lv_coord_t max_width = LV_HOR_RES * 90 / 100;
    lv_coord_t max_height = LV_VER_RES * 62 / 100;
    lv_coord_t zoom_w = (max_width * 256) / img_width;
    lv_coord_t zoom_h = (max_height * 256) / img_height;
    lv_coord_t zoom = (zoom_w < zoom_h) ? zoom_w : zoom_h;
    if (zoom > 256) {
    ...
    if (bubble != nullptr && lv_obj_is_valid(bubble)) {
        lv_obj_set_size(bubble, scaled_width + lvgl_theme->spacing(4), scaled_height + lvgl_theme->spacing(4));
        lv_obj_center(preview_image_);
        lv_obj_t* row = lv_obj_get_parent(bubble);
        if (row != nullptr && lv_obj_is_valid(row)) {
            lv_obj_set_height(row, scaled_height + lvgl_theme->spacing(6));
            lv_obj_align(bubble, LV_ALIGN_CENTER, 0, 0);
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    const bool is_system = strcmp(role, "system") == 0;
    ...
    lv_obj_t* bubble = lv_obj_create(row);
    lv_obj_set_style_radius(bubble, is_system ? 10 : 18, 0);
    lv_obj_set_style_border_width(bubble, 0, 0);
    lv_obj_set_style_pad_top(bubble, lvgl_theme->spacing(3), 0);
    lv_obj_set_style_pad_bottom(bubble, lvgl_theme->spacing(3), 0);
    lv_obj_set_style_pad_left(bubble, lvgl_theme->spacing(4), 0);
    lv_obj_set_style_pad_right(bubble, lvgl_theme->spacing(4), 0);
    lv_obj_set_scrollbar_mode(bubble, LV_SCROLLBAR_MODE_OFF);
    ...
    lv_coord_t max_width = LV_HOR_RES * (is_system ? 76 : 72) / 100;
    lv_coord_t min_width = LV_HOR_RES * 12 / 100;
    ...
    if (is_user) {
        bubble_color = lv_color_hex(0x007AFF);  // iMessage blue
        text_color = lv_color_white();
        lv_obj_set_user_data(bubble, (void*)"user");
    } else if (is_system) {
        bubble_color = lvgl_theme->system_bubble_color();
        text_color = lvgl_theme->system_text_color();
        lv_obj_set_user_data(bubble, (void*)"system");
    } else {
        bubble_color = (current_theme_ && current_theme_->name() == "dark") ? lv_color_hex(0xE5E7EB) : lv_color_hex(0xFFFFFF);
        text_color = lv_color_hex(0x0B1220);
    ...
    lv_obj_set_width(bubble, text_width + lvgl_theme->spacing(8));
    lv_obj_set_height(bubble, LV_SIZE_CONTENT);
    lv_obj_update_layout(bubble);
    lv_obj_set_height(row, lv_obj_get_height(bubble) + lvgl_theme->spacing(2));

    if (is_user) {
        lv_obj_align(bubble, LV_ALIGN_RIGHT_MID, -lvgl_theme->spacing(3), 0);
    } else if (is_system) {
        lv_obj_align(bubble, LV_ALIGN_CENTER, 0, 0);
    } else {
        lv_obj_align(bubble, LV_ALIGN_LEFT_MID, lvgl_theme->spacing(3), 0);
    }
    ...
    while (lv_obj_get_child_cnt(content_) > MYAI_MAX_CHAT_ITEMS) {
        ...
        if (preview_image_ != nullptr && lv_obj_is_valid(preview_image_)) {
            lv_obj_t* preview_bubble = lv_obj_get_parent(preview_image_);
            lv_obj_t* preview_row = preview_bubble != nullptr ? lv_obj_get_parent(preview_bubble) : nullptr;
            if (preview_row == first_child) {
                preview_image_ = nullptr;
                preview_image_cached_.reset();
            }
        }

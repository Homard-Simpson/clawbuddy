...
namespace {
std::string TrimString(const char* value) {
    if (value == nullptr) {
        return "";
    }
...
bool IsHttpUrlOrBlank(const std::string& value) {
    return value.empty() || value.rfind("http://", 0) == 0 || value.rfind("https://", 0) == 0;
}

bool IsWebSocketUrlOrBlank(const std::string& value) {
...
std::string LoadNvsString(const char* ns, const char* key) {
    ...
    size_t size = 0;
    err = nvs_get_str(nvs, key, nullptr, &size);
...
esp_err_t SaveOrEraseNvsString(const char* ns, const char* key, const std::string& value) {
    nvs_handle_t nvs;
...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    // Voice server URL is stored in the websocket namespace and is optional.
    websocket_url_ = LoadNvsString("websocket", "url");
}
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t advanced_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // Validate friendly setup fields first. Blank values intentionally fall back
            // to the firmware/OTA-provided defaults.
            cJSON *ota_url = cJSON_GetObjectItem(json, "ota_url");
            std::string new_ota_url = cJSON_IsString(ota_url) ? TrimString(ota_url->valuestring) : "";
            if (!IsHttpUrlOrBlank(new_ota_url)) {
                cJSON_Delete(json);
                httpd_resp_set_type(req, "application/json");
                httpd_resp_send(req, "{\"success\":false,\"error\":\"OTA URL must start with http:// or https://, or be blank for the build default\"}", HTTPD_RESP_USE_STRLEN);
                return ESP_OK;
            }

            cJSON *websocket_url = cJSON_GetObjectItem(json, "websocket_url");
            std::string new_websocket_url = cJSON_IsString(websocket_url) ? TrimString(websocket_url->valuestring) : "";
            if (!IsWebSocketUrlOrBlank(new_websocket_url)) {
                cJSON_Delete(json);
                httpd_resp_set_type(req, "application/json");
                httpd_resp_send(req, "{\"success\":false,\"error\":\"WebSocket URL must start with ws:// or wss://, or be blank for OTA server configuration\"}", HTTPD_RESP_USE_STRLEN);
                return ESP_OK;
            }
            ...
            // 保存OTA URL
            this_->ota_url_ = new_ota_url;
            if (this_->ota_url_.empty()) {
                err = nvs_erase_key(nvs, "ota_url");
                if (err == ESP_ERR_NVS_NOT_FOUND) {
                    err = ESP_OK;
                }
            } else {
            ...
            if (err == ESP_OK) {
                this_->websocket_url_ = new_websocket_url;
                err = SaveOrEraseNvsString("websocket", "url", this_->websocket_url_);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to save websocket URL: %d", err);
                }
            }
            cJSON_Delete(json);

...
static constexpr int V2_PANEL_X_GAP=0x10;
...
// V2 hardware requirement from Waveshare's minimal CO5300/CST820 display examples.
// The AMOLED/reset lines use TCA9554 outputs 0, 1, and 2 at 0x20 and must be
// asserted before the first CO5300 command. Doing this after panel_init is too late.
static void v2_panel_power_reset(){
  ensure_i2c_bus();
...
static void init_rest(){
  ...
  axp_dump("i2c-up"); ESP_LOGI(TAG,"PHASE3: i2c bus + PMU dump done"); vTaskDelay(pdMS_TO_TICKS(2000));
  ...
  ESP_LOGI(TAG,"PHASE4: PSRAM fb allocated"); vTaskDelay(pdMS_TO_TICKS(2000));
  splash_bars(); ESP_LOGI(TAG,"PHASE5: bars redrawn via app path"); vTaskDelay(pdMS_TO_TICKS(3000));
  ESP_ERROR_CHECK(esp_lcd_panel_co5300_set_brightness(panel,75)); ESP_LOGI(TAG,"PHASE6: brightness 75 applied"); vTaskDelay(pdMS_TO_TICKS(2000));
  esp_lcd_panel_io_handle_t touch_io; esp_lcd_panel_io_i2c_config_t touch_io_cfg={};touch_io_cfg.dev_addr=ESP_LCD_TOUCH_IO_I2C_CST816S_ADDRESS;touch_io_cfg.control_phase_bytes=1;touch_io_cfg.lcd_cmd_bits=8;touch_io_cfg.flags.disable_control_phase=1;touch_io_cfg.scl_speed_hz=400000; ESP_ERROR_CHECK(esp_lcd_new_panel_io_i2c(bus,&touch_io_cfg,&touch_io)); esp_lcd_touch_config_t touch_cfg={.x_max=W,.y_max=H,.rst_gpio_num=GPIO_NUM_NC,.int_gpio_num=GPIO_NUM_21,.levels={.reset=0,.interrupt=0},.flags={.swap_xy=0,.mirror_x=0,.mirror_y=0}}; ESP_ERROR_CHECK(esp_lcd_touch_new_i2c_cst816s(touch_io,&touch_cfg,&touch));
  axp_dump("touch"); ESP_LOGI(TAG,"PHASE7: touch ready"); vTaskDelay(pdMS_TO_TICKS(2000));
...
extern "C" void app_main(){
  phase1_display_bars();
  ESP_LOGI(TAG,"PHASE1 hold 5s"); vTaskDelay(pdMS_TO_TICKS(5000));
  ...
  ESP_LOGI(TAG,"PHASE2: nvs done"); vTaskDelay(pdMS_TO_TICKS(2000));
  init_rest();
